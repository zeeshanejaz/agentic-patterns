"""Planning with MAF Agents in a plan → execute → replan loop. Traces via OTEL."""
...
def build_agents() -> dict[str, Agent]:
    client = _client()
    return {
        "planner": Agent(client=client, name="planner", instructions=PLAN_SYSTEM),
        "repair": Agent(client=client, name="repair", instructions=PLAN_REPAIR_SYSTEM),
        "step": Agent(client=client, name="step", instructions=PLAN_STEP_SYSTEM),
        "replan": Agent(client=client, name="replan", instructions=PLAN_REPLAN_SYSTEM),
    }
...
async def make_plan(agents: dict[str, Agent], email: str) -> Plan:
    raw = await _complete(agents["planner"], email)
    try:
...
async def execute_step(
    ...
    output = await _complete(agents["step"], user)
...
async def replan_remaining(
    ...
    raw = await _complete(agents["replan"], user)
    try:
...
async def _reply_from_execution(
    agents: dict[str, Agent],
    email: str,
    ...
    return await _complete(agents["step"], user)


async def run(email: str) -> PlanningResult:
    _ensure_otel()
    agents = build_agents()
    with get_tracer().start_as_current_span("pattern.planning") as span:
        span.set_attribute("pattern", "planning")
        span.set_attribute("backend", "maf")
        plan = await make_plan(agents, email)
        ...
        while queue:
            step = queue[0]
            queue = queue[1:]
            result = await execute_step(agents, email, step, results)
            results.append(result)
            if result.status == "blocked" and replans < MAX_REPLANS and queue:
                queue = _ready_order(await replan_remaining(agents, email, results, queue))
                executed_ids = {item.id for item in results}
                steps = [s for s in steps if s.id in executed_ids] + queue
                replans += 1
        reply = await _reply_from_execution(agents, email, results, steps)
